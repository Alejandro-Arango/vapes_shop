...
class CapacityConfigTests(unittest.TestCase):
    ...
    def test_migrate_without_hardened_tmpfs_is_rejected(self):
        compose_text = (PROJECT_ROOT / "compose.yaml").read_text(
            encoding="utf-8",
        )
        invalid_text = compose_text.replace(
        ...
        self.assertIn(
            "migrate no monta /tmp como tmpfs endurecido",
            findings,
        )
    ...
    def test_production_check_without_throttle_quantity_guard_is_rejected(self):
        production_check_text = (
            PROJECT_ROOT
            / "backend"
            / "store"
        ...
        invalid_text = production_check_text.replace(
            ...
            "MAX_THROTTLE_QUANTITY_UNBOUNDED",
        )

        findings = capacity.validate_production_check_security(invalid_text)

        self.assertIn(
            "production_check.py no contiene MAX_THROTTLE_QUANTITY_DIGITS",
            findings,
        )
    ...
    def test_production_check_without_inline_script_guard_is_rejected(self):
        production_check_text = (
            PROJECT_ROOT
            / "backend"
            / "store"
    ...
    def test_media_proxy_without_symlink_guard_is_rejected(self):
        nginx_text = (
            PROJECT_ROOT / "docker" / "nginx.conf"
        ).read_text(encoding="utf-8")
        invalid_text = nginx_text.replace(
    ...
    def test_resilience_config_without_gunicorn_access_log_is_rejected(self):
        start_text = (
            PROJECT_ROOT / "docker" / "start.sh"
        ).read_text(encoding="utf-8")
        nginx_text = (
        ...
        self.assertIn(
            "docker/start.sh no contiene --access-logfile -",
            findings,
        )

    def test_resilience_config_without_gunicorn_tmp_dir_is_rejected(self):
        start_text = (
            PROJECT_ROOT / "docker" / "start.sh"
        ).read_text(encoding="utf-8")
        nginx_text = (
        ...
        self.assertIn(
            "docker/start.sh no contiene --worker-tmp-dir /tmp",
            findings,
        )
    ...
    def test_nginx_without_health_cache_bypass_is_rejected(self):
        start_text = (
            PROJECT_ROOT / "docker" / "start.sh"
        ).read_text(encoding="utf-8")
        nginx_text = (
    ...
    def test_nginx_without_upstream_cache_header_hide_is_rejected(self):
        start_text = (
            PROJECT_ROOT / "docker" / "start.sh"
        ).read_text(encoding="utf-8")
        nginx_text = (
        ...
        self.assertIn(
            "docker/nginx.conf no contiene proxy_hide_header Cache-Control;",
            findings,
        )
    ...
    def test_production_monitor_without_health_path_guard_is_rejected(self):
        monitor_text = (
            PROJECT_ROOT / "scripts" / "monitor_production.py"
        ).read_text(encoding="utf-8")
        invalid_text = monitor_text.replace(
    ...
    def test_production_monitor_without_content_type_guard_is_rejected(self):
        monitor_text = (
            PROJECT_ROOT / "scripts" / "monitor_production.py"
        ).read_text(encoding="utf-8")
        invalid_text = monitor_text.replace(
            "Content-Type JSON",
            "tipo de contenido valido",
        )
    ...
    def test_host_readiness_without_active_security_updates_is_rejected(self):
        inputs = self.host_provisioning_inputs()
        inputs["readiness_text"] = inputs["readiness_text"].replace(
    ...
    def test_host_readiness_without_package_list_timer_is_rejected(self):
        inputs = self.host_provisioning_inputs()
        inputs["readiness_text"] = inputs["readiness_text"].replace(
    ...
    def test_host_readiness_without_active_package_list_timer_is_rejected(
        self,
    ):
        inputs = self.host_provisioning_inputs()
        inputs["readiness_text"] = inputs["readiness_text"].replace(
